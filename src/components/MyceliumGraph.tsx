...
function InteractiveSomaticSphere({
  ...
  const handlePointerUp = (e: any) => {
    ...
    // Click: small movement (<25px) and short duration (<600ms)
    if (distance < 25 && duration < 600) {
      onClick(node);
    // Long press: small movement but longer duration
    } else if (distance < 25 && duration >= 600) {
      onLongSelect?.(node, e.clientX, e.clientY);
    }
    // drag: already handled in handlePointerMove (distance >= 10)
  };
