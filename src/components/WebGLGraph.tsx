...
import { getEpochNumberForNode } from '../utils';

// Re-export VisibleLayers from MyceliumGraph so consumers can import from either file
export type { VisibleLayers } from '../utils';
import type { VisibleLayers } from '../utils';
...
export default function WebGLGraph(props: MyceliumGraphProps) {
  ...
  useEffect(() => {
    ...
    if (!ok) {
      ...
      const hasWebGL1 = !!testCanvas.getContext('webgl');
      const hasWebGL2 = !!testCanvas.getContext('webgl2');
      const msg = !hasWebGL2
        ? (hasWebGL1 ? 'WebGL2 not available, only WebGL1. Use a modern browser.' : 'WebGL not available. Use a modern browser with GPU support.')
        : 'WebGL2 supported but engine failed to initialize. Check browser console (F12) for shader errors.';
      setGlError(msg);
      return;
    }
    ...
    // Initial size — use window dimensions as fallback
    ...
    const loop = () => {
      try {
        ...
        console.error('[WebGLGraph] Render loop error:', e);
      }
  ...
  const _updateLabels = useCallback((
    ...
    for (const node of filteredNodes) {
      ...
      // Use renderer's internal physics coordinates for accurate label positions
      const phys = renderer.getNodePhysicsPosition(node.id);
      if (!phys) continue;
      // Offset label above the node (Y+ in scene coords)
      const nx = phys[0];
      const ny = phys[1] + 0.25; // Float above the node
      const nz = phys[2];
      ...
      // Depth-based opacity: nodes far from camera are more transparent
      const camDist = camera.distance;
      const depthFade = Math.min(1, Math.max(0.3, 1 - (camDist - 15) / 60));

      const fontSize = (isSelected || isActiveAudio || node.status === 'rooted') ? 12 : 10;
      const lvl = (node.level === 'macro' ? 1.0 : node.level === 'meso' ? 0.85 : 0.7) * depthFade;
    ...
    >
      ...
      {glError && (
        <div style={{
          ...
          background: '#0a0a0f', color: '#DFB757', fontFamily: 'Inter, sans-serif',
          padding: '40px', zIndex: 100, gap: '16px',
        }}>
          <div style={{ fontSize: '24px', fontWeight: 600 }}>⚠️</div>
          <div style={{ fontSize: '14px', color: '#888', textAlign: 'center', maxWidth: '300px' }}>
            {glError}
          </div>
          <button onClick={() => window.location.reload()}
            style={{
              padding: '8px 24px', borderRadius: '8px', border: '1px solid #DFB757',
              background: 'transparent', color: '#DFB757', cursor: 'pointer', fontSize: '13px'
            }}>
            Retry
          </button>
        </div>
