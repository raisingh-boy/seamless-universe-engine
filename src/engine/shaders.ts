...
void main() {
  ...
  float selMult = a_selected > 0.5 ? 2.5 : 1.0;
  float pointSize = a_size * breathe * 8.0 * selMult;
...
void main() {
  ...
  float alpha = glow * 0.5 * breathe * v_color.a * selBoost;
...
void main() {
  ...
  vec4 viewPos = u_viewProj * vec4(a_pos, 1.0);
  float depthFactor = clamp(1.0 - abs(viewPos.z) * 0.005, 0.3, 1.0);

  float breathe = 1.0 + sin(u_time * 0.8 + a_phase) * 0.1;
  float selMult = a_selected > 0.5 ? 1.5 : 1.0;
  float pointSize = a_size * breathe * depthFactor * selMult;
