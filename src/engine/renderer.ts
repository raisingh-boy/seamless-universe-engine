...
export class GraphRenderer {
  ...
  setGraphData(
    ...
    this.physicsNodes = nodes.map((n, idx) => {
      ...
      // Start closer to target (spread reduced from 18→3 so physics stabilizes fast)
      let x = ex ? ex.x : tx + (seed1 - 0.5) * 3;
      let y = ex ? ex.y : ty + (seed2 - 0.5) * 3;
      let z = ex ? ex.z : tz + (Math.random() - 0.5) * 4;
      ...
      const glowC: [number,number,number,number] = [domColor[0], domColor[1], domColor[2], 0.85];
  ...
  tickPhysics(time: number, ascendingNodeId?: string | null): void {
    ...
    const gravityStrength = 0.065;
    const repulsionStrength = 1300;
    const attractionStrength = 0.025;
    const damping = 0.88;
    ...
    nodes.forEach((node, idx) => {
      if (node.id === 'central-me') {
        ...
        // Living wave sway (reduced from 0.16→0.06 for stability)
        node.x += Math.sin(time * 0.8 + idx * 0.7) * 0.06;
        node.y += Math.cos(time * 0.6 + idx * 0.4) * 0.06;
        node.z += Math.sin(time * 1.0 + idx * 0.9) * 0.04;
      }
  ...
  getNodePhysicsPosition(nodeId: string): [number,number,number] | null {
    const node = this.physicsNodes.find(n => n.id === nodeId);
    if (!node) return null;
    return [node.x * 0.045, node.y * 0.045, node.z * 0.045];
  }
  ...
  private _updateNodeBuffers(camera: Camera) {
    ...
    const pxScale = 1200 / camDist; // Increased from 600 — nodes were invisible (2px)

    for (let i = 0; i < n; i++) {
      const nd = nodes[i];
      const sizeInPx = nd.baseSize * pxScale; // Use baseSize directly, not double-scaled
