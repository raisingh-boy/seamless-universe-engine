...
export class Camera {
  theta = 0;
  phi = 0.6;  // Dramatic tilt for 3D depth
  distance = 30;
  ...
  attachListeners(): () => void {
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (isRightDrag) {
        ...
        // Orbit — increased sensitivity for more responsive feel
        this.theta -= dx * 0.008;
        this.phi = Math.max(-Math.PI * 0.48, Math.min(Math.PI * 0.48, this.phi + dy * 0.008));
      }
      this._fireChange();
    };
    ...
    const onWheel = (e: WheelEvent) => {
      e.preventDefault();
      // Smoother zoom scaling
      const speed = 0.06;
      const delta = 1.0 + (e.deltaY > 0 ? -speed : speed);
      this.distance = Math.min(80, Math.max(2, this.distance * (1 + (delta - 1) * 1.5)));
      this._fireChange();
    };
