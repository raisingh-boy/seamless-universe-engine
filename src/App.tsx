...
import { fetchNodes, fetchEdges, resonateNode, chatWithAI } from './api/universeApi';
import { playNodeTone, playResonanceChord, startAmbientDrone, stopAmbientDrone } from './api/nodeSounds';
import { 
...
export default function App() {
  ...
  const [showOnboarding, setShowOnboarding] = useState<boolean>(() => {
    if (typeof window !== 'undefined' && localStorage.getItem('seamless-onboarding-seen')) {
      return false;
    }
    return false; // Always off for now — kan onboarding later
  });
  ...
  useEffect(() => {
    async function loadFromAPI() {
      try {
        const apiNodes = await fetchNodes({ world: 'atlas' });
        const apiEdges = await fetchEdges({ world: 'atlas' });
        if (apiNodes.length > 0) {
          // Merge API nodes with static nodes (field + ghost + me stay static)
          const staticFieldNodes = INITIAL_NODES.filter(n => n.world !== 'atlas');
          setNodes([...apiNodes, ...staticFieldNodes]);
          setLinks(apiEdges);
          setApiLoaded(true);
        }
      } catch (err) {
        console.warn('API load failed, using static data:', err);
        setApiError('API unavailable, using static data');
      }
    }
    loadFromAPI();
  }, []);

  // Start ambient drone on first user interaction
  useEffect(() => {
    const handler = () => {
      startAmbientDrone();
  ...
  // AI Chat handler (defined after all state declarations for TS read-before-declare safety)
  const sendAIChat = () => {
    ...
    (async () => {
      try {
        const ctx = selectedNode ? `${selectedNode.nameEn || selectedNode.nameRu}` : undefined;
        const res = await chatWithAI(userMsg, ctx);
        setAiChatMessages(prev => [...prev, { role: 'assistant', content: res.response || '...' }]);
      } catch (err) {
        setAiChatMessages(prev => [...prev, { role: 'assistant', content: 'AI unavailable. Make sure Ollama is running.' }]);
      }
      setAiChatLoading(false);
    })();
  };

  // Update handleNodeResonated to use API too
  const handleNodeResonatedAPI = (nodeId: string) => {
    resonateNode(nodeId, userEmail || undefined).catch(() => {});
  };
  ...
  const [showStarryOpening, setShowStarryOpening] = useState<boolean>(false);
  useEffect(() => {
    const timer = setTimeout(() => setShowStarryOpening(false), 7000);
  ...
  const handleNodeResonated = (nodeId: string) => {
    ...
    // Play sound for this domain
    const node = nodes.find(n => n.id === nodeId);
    if (node) playResonanceChord(node.domain);
  ...
  return (
    <div className="relative w-screen h-screen flex flex-col bg-[#050505] text-[#E0D8D0] overflow-hidden select-none font-sans" id="seamless-cosmos-hub">
      ...
      <header className={`absolute top-0 left-0 right-0 h-16 bg-gradient-to-b from-[#050505] via-[#050505]/95 to-transparent flex items-center justify-between px-6 z-30 select-none pointer-events-auto transition-all duration-1000 ${vibeMode === 'cinematic' ? 'opacity-15 hover:opacity-100 hover:duration-200' : 'opacity-100'}`}>
        ...
        {/* Global toggles bar (Vibe mode + Language) */}
        <div className="flex items-center gap-2">
          {/* Vibe mode quick toggle - visible always */}
          <button
            onClick={() => {
              const next = { colour: 'mono' as const, mono: 'cinematic' as const, cinematic: 'colour' as const };
              setVibeMode(prev => next[prev]);
              showFlash(vibeMode === 'colour' ? (language === 'ru' ? '🔘 Монохром' : '🔘 Monochrome') : vibeMode === 'mono' ? (language === 'ru' ? '🎬 Кинорежим' : '🎬 Cinematic') : (language === 'ru' ? '🎨 Цветной режим' : '🎨 Colour'));
            }}
            className={`w-10 h-8 rounded-lg border flex items-center justify-center text-xs cursor-pointer transition-all active:scale-95 ${
              vibeMode === 'mono' ? 'bg-gray-200 text-gray-900 border-gray-300' : vibeMode === 'cinematic' ? 'bg-indigo-500/10 text-indigo-300 border-indigo-500/30' : 'bg-white/5 text-white border-white/5 hover:bg-white/10'
            }`}
            title={language === 'ru' ? 'Режим: Цвет/Моно/Кино' : 'Vibe: Colour/Mono/Cinematic'}
          >
            {vibeMode === 'mono' ? '◑' : vibeMode === 'cinematic' ? '◐' : '◐'}
          </button>
          {/* RU / EN switcher */}
          <button
      ...
      <main className="flex-1 w-full h-full relative pb-16" id="universe-main-area">
          ...
          <div className="flex gap-1.5">
            ...
            <button
               ...
               className={`w-11 h-11 rounded-xl bg-[#ffffff08]/85 backdrop-blur-md border flex items-center justify-center transition-all cursor-pointer ${
                 showAIChat ? 'border-emerald-500 text-emerald-400' : 'border-[#ffffff10] text-gray-400 hover:text-white hover:bg-white/5'
               }`}
               title={language === 'ru' ? 'AI-собеседник' : 'AI Companion'}
            >
        ...
        <WebGLGraph
          ...
          language={language}
          onNodeSelect={(n) => { setSelectedNode(n); playNodeTone(n.domain || 'hybrid', 0.2); }}
      ...
      {/* AI Chat Overlay */}
      {showAIChat && (
        <div className="fixed bottom-24 left-4 w-[360px] max-h-[420px] bg-[#0E1528]/98 border border-white/10 backdrop-blur-md rounded-2xl shadow-2xl z-40 flex flex-col overflow-hidden animate-slide-in">
          <div className="flex items-center justify-between p-3 border-b border-white/5">
            <span className="text-[10px] font-mono tracking-widest text-emerald-400 uppercase flex items-center gap-2">
              ...
              {language === 'ru' ? 'AI-Проводник' : 'AI Guide'}
              {aiChatLoading && <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"></span>}
            </span>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 max-h-[300px] custom-scrollbar">
            {aiChatMessages.length === 0 && (
              <div className="text-center text-gray-500 text-[10px] font-mono py-6">
                {language === 'ru' 
                  ? 'Спроси меня о любой ноде. Например: "Что такое соматика?"'
                  : 'Ask me about any node. Try: "What is somatics?"'}
              </div>
            )}
            ...
            {aiChatLoading && (
              <div className="flex justify-start">
                <div className="bg-white/5 px-3 py-2 rounded-2xl text-xs text-gray-400 border border-white/5">
          ...
          <div className="p-2 border-t border-white/5">
            <div className="flex gap-2">
              <input
                ...
                placeholder={language === 'ru' ? 'Спроси о Seamless Universe...' : 'Ask about Seamless Universe...'}
                className="flex-1 bg-white/5 text-xs text-white px-3 py-2 rounded-xl border border-white/10 focus:border-emerald-500/50 focus:outline-none placeholder-gray-600"
              />
              <button
                onClick={sendAIChat}
                disabled={aiChatLoading || !aiChatInput.trim()}
                className="px-3 py-2 rounded-xl bg-emerald-600/80 hover:bg-emerald-500 text-white text-xs disabled:opacity-30 transition-all cursor-pointer"
              >
                {language === 'ru' ? '→' : '→'}
              </button>
            </div>
            {selectedNode && (
              <div className="mt-1.5 text-[8px] text-gray-600 font-mono truncate">
                {language === 'ru' ? 'Контекст' : 'Context'}: {selectedNode.nameEn || selectedNode.nameRu}
              </div>
            )}
          </div>
        </div>
    ...
    <footer className={`fixed bottom-0 left-0 right-0 h-16 border-t border-[#ffffff10] bg-[#050505]/95 backdrop-blur-md flex items-center justify-between px-6 z-30 transition-all duration-1000 ${vibeMode === 'cinematic' ? 'opacity-15 hover:opacity-100 hover:duration-200' : 'opacity-100'}`}>
